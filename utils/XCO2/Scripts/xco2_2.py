...
loc = locals()
exec("from config.tracers_input_" + CaseName + " import FFE_dic, FTA_dic, BCK_dic, ANA_dic")
FFE_dic = loc["FFE_dic"]
FTA_dic = loc["FTA_dic"]
BCK_dic = loc["BCK_dic"]
ANA_dic = loc["ANA_dic"]
...
#VarList = ["CO2_TOT", "CO2_BCK", "CO2_VEGAS", "CO2_FFE"]
VarList = []
VarList += [FFE_dic[varName]["ConcName"] for varName in FFE_dic]
VarList += [FTA_dic[varName]["ConcName"] for varName in FTA_dic]
VarList += [BCK_dic[varName]["ConcName"] for varName in BCK_dic]
VarList += [ANA_dic[varName]["ConcName"] for varName in ANA_dic]
...
if __name__ == "__main__":
    ...
    while(Current <= End):
        ...
        dsIn = nc.Dataset(InDir + "/" + InPrefix + "_" + DomName + "_" + Current.strftime("%Y-%m-%d_%H:%M:%S"))
        time = dsIn.variables["XTIME"][0].filled(np.nan)
        lon = dsIn.variables["XLONG"][0,0,:].filled(np.nan)
        lat = dsIn.variables["XLAT"][0,:,0].filled(np.nan)
        eta_old = dsIn.variables["ZNU"][0].filled(np.nan)
        ...
        for ivar in VarList:
            print("  ==> " + ivar)
            var_data = dsIn.variables[ivar][0].filled(np.nan)
        ...
        dsOut["time"].attrs["units"] = dsIn.variables["XTIME"].units
