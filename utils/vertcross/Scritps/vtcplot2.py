...
def draw_2D(fig, ax, proj, co2, lon, lat, title = None, uv_dic = None, uv_set = None, vmin = 400, vmax = 700, cbar_orn = "vertical", sections_dic = None):
    ...
    cbar = fig.colorbar(cs, orientation = cbar_orn)
    cbar.set_label("ppm")
...
while(Current <= End):
    ...
    ax2 = plt.subplot2grid((3,4), (0,0), rowspan = 2, colspan = 2, fig = fig, projection = proj)
    ...
    draw_2D(fig, ax2, proj, co2, lon, lat, title = "Surface CO2", uv_dic = {"U": u, "V": v}, uv_set = {"xint": 19, "yint": 19, "windcolor": "white"}, vmin = vmin_sur, vmax = vmax_sur, cbar_orn = "vertical", sections_dic = sections_dic)
...
#    sup_title = section + " - " + VarPlot + "\n" + Current.strftime("%Y-%m-%d_%H:%M:%S") + " UTC\n" + (Current + dt_UTC).strftime("%Y-%m-%d_%H:%M:%S") + " LST"
    sup_title = section + " - " + VarPlot + "\n" + (Current + dt_UTC).strftime("%Y-%m-%d_%H:%M:%S") + " LST"
    fig.suptitle(sup_title, y = 0.95, fontsize = 18)
    #plt.show()
    plt.savefig(PlotDir + "/vtc2_" + section + "_" + VarPlot + "_" + Current.strftime("%Y-%m-%d_%H:%M:%S") + ".png")
    plt.close(fig)
    Current += dt
